...
/obj/effect/decal/metal_splash/Initialize(ml, _mat = MATERIAL_WASTE, _amt = 100)
	. = ..(ml)
	for(var/obj/effect/decal/metal_splash/splash in loc)
		...
		for(var/thing in splash.matter)
			if(!highest_mat || splash.matter[thing] > splash.matter[highest_mat])
				highest_mat = thing
		if(highest_mat != splash.material.name)
			splash.material = SSmaterials.get_material_by_name(highest_mat)
	...
	material = SSmaterials.get_material_by_name(_mat)
