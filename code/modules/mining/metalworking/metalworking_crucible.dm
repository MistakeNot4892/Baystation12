...
/obj/structure/metalworking/crucible/on_update_icon()
	...
	if(LAZYLEN(matter_contents))
		...
		if(last_highest_mat)
			var/material/mat = SSmaterials.get_material_by_name(last_highest_mat)
			if(mat)
...
/obj/structure/metalworking/crucible/attack_hand(var/mob/user)
	...
	for(var/mat in matter_contents)
		if(mat == MATERIAL_WASTE)
			continue
		if(!primary_material || matter_contents[mat] > matter_contents[primary_material])
			primary_material = mat
	for(var/mat in matter_contents)
		if(mat != primary_material && mat != MATERIAL_WASTE)
			matter_contents[MATERIAL_WASTE] = matter_contents[MATERIAL_WASTE] + matter_contents[mat]
			matter_contents -= mat
	...
	for(var/mob/living/M in pour_to)
		var/material/mat = SSmaterials.get_material_by_name(primary_material)
	...
	if(locate(/obj/structure/metalworking/casting_basin) in pour_to)
		if(LAZYLEN(matter_contents))
			...
			to_chat(user, SPAN_NOTICE("The empty [src.name] clunks hollowly as you set it upright."))
		return
	else
		if(LAZYLEN(matter_contents))
	...
	if(donor)
		...
		if(LAZYLEN(attempt_to_alloy))
			...
			for(var/thing in making_alloys)
				...
				new_metals[M.name] = new_metals[M.name] + (making * M.units_per_sheet)
