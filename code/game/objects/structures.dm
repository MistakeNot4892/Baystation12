...
/obj/structure/proc/update_connections(propagate = 0)
	...
	for(var/direction in GLOB.cardinal)
		...
		var/success = 0
		for(var/b_type in GLOB.default_blend_objects)
			if(istype(T, b_type))
				success = 1
				if(propagate)
		...
		if(!success)
			for(var/obj/O in T)
				for(var/b_type in GLOB.default_blend_objects)
					if(istype(O, b_type))
						...
						for(var/obj/structure/S in T)
							if(istype(S, src))
								success = 0
						for(var/nb_type in GLOB.default_noblend_objects)
							if(istype(O, nb_type))
								success = 0
					if(success)
						break
				if(success)
					break
		if(success)
			LAZYADD(dirs, get_dir(src, T))
			LAZYADD(other_dirs, get_dir(src, T))
